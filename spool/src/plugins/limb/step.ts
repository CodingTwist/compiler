// Feet: body-frame points to world, planting, and stepping to the ground.
import {
  BLOCK_TAGS,
  Block,
  NbtPath,
...
export function readBody(s: LimbState, ctx: FunctionContext): void {
  ...
  ctx
    .execute()
    .storeResultScore(s.yaw)
    .run((c) => c.entity(Selector.self()).get(NbtPath("Rotation[0]"), MM));
...
/** Writes world point `p` into {@link LimbState.local}: measured from the rig's seat, on world axes. */
function toLocal(s: LimbState, ctx: FunctionContext, p: ScoreVec3): void {
  math`${p} - ${s.body}`.into(s.local, ctx);
  s.local.y.remove(s.opts.mountY ?? 0, ctx);
}

/** Counts each group's stepping legs and counts down their holds, for the gait gate. */
export function countSteps(s: LimbState, ctx: FunctionContext): void {
  s.stepping.forEach((g) => g.set(0, ctx));
  s.held.forEach((h) => ctx.if(h.atLeast(1), (c) => h.remove(1, c)));
  s.opts.legs.forEach((leg, i) =>
    ctx.if(s.legs[i].clock.atLeast(1), (c) => s.stepping[leg.group].add(1, c)),
  );
}
...
export function plant(s: LimbState, ctx: FunctionContext): void {
  s.opts.legs.forEach((leg, i) => {
    toWorld(s, ctx, leg.rest, s.legs[i].foot);
    // Set, since an unset score fails `matches 0`.
    s.legs[i].clock.set(0, ctx);
  });
}
 ...
 * Called from {@link step}, which leaves the world rest point in `world` and the drift² in `w`.
 */
export function buildProbe(
  ...
  const { group } = s.opts.legs[i];
  const leg = s.legs[i];
  const loc = s.locator;
  const reach = s.opts.overshoot ?? s.opts.stride ?? 0.9;
  // Past the rest point along the drift, or a walking body outruns feet that land where it was.
  math`${s.world.x} + (${s.world.x} - ${leg.foot.x}) * ${reach} / sqrt(${s.w})`.into(
    s.world.x,
    ctx,
  );
  math`${s.world.z} + (${s.world.z} - ${leg.foot.z}) * ${reach} / sqrt(${s.w})`.into(
    s.world.z,
    ctx,
  );
  math`${s.body.y} + 2`.into(s.world.y, ctx);
  loc.moveTo(ctx, s.world);
  for (let n = 0; n < PROBE_DEPTH; n++)
    ctx
      .execute()
      .at(loc.selector())
      .ifBlock(Pos.rel(0, -1, 0), AIR)
      .run((b) => b.teleport(loc.selector(), Pos.rel(0, -1, 0)));
  ctx
    .execute()
    .at(loc.selector())
...
export function step(
  ...
  toWorld(s, ctx, rest, s.world);
  const { w } = s;
  math`(${foot.x} - ${s.world.x}) * (${foot.x} - ${s.world.x}) + (${foot.z} - ${s.world.z}) * (${foot.z} - ${s.world.z})`.into(
    w,
    ctx,
  );
  const free = and(
    clock.equal(0),
    s.stepping[1 - group].equal(0),
    not(s.held[group].atLeast(1)),
  );
  ctx.if(and(free, w.greaterThan(stride * stride)), (c) => c.call(probe));
  ctx.if(clock.atLeast(1), (c) => {
    ...
    // Landed: sit out the next poll, or this group, checked first, would always step again first.
    c.if(clock.equal(0), (l) => s.held[group].set(2, l));
  });
  toLocal(s, ctx, foot);
}
