...
function attachBody(dp: Datapack): string {
  const hit = [...dp.files.entries()].find(([, v]) =>
    v.includes("tag @s add grappling"),
  );
...
describe("dp.grapple (kit)", () => {
  it("registers start/drive/constrain/tick/stop + the web ray, and tick-tags the loop", () => {
    const { dp, g } = build();
    for (const f of [
      "zzzplugin/grapple/start",
      "zzzplugin/grapple/drive",
      "zzzplugin/grapple/constrain",
      "zzzplugin/grapple/tick",
      "zzzplugin/grapple/stop",
    ]) {
      expect(dp.files.has(f)).toBe(true);
    }
    // The raycast marcher lives in the `raycast` plugin's own namespace now.
    expect(dp.files.has("zzzplugin/grapple/web")).toBe(true);
    expect(dp.tags.get("tick")?.has("zzzplugin/grapple/tick")).toBe(true);
    expect(g.start.getName()).toBe("zzzplugin/grapple/start");
    expect(g.stop.getName()).toBe("zzzplugin/grapple/stop");
  });

  it("pulls in player_motion (its api functions are present)", () => {
    const { dp } = build();
    expect(dp.files.has("zzzplugin/player_motion/launch_global_xyz")).toBe(true);
    expect(dp.files.has("zzzplugin/player_motion/launch/main")).toBe(true);
  });

  /** The web ray's loop: its check, then its pass and exit functions. */
  const LOOP = "zzzplugin/grapple/web/return_0/while_0";
  ...
  it("the web ray (raycast plugin) marches along ^ while air remains and anchors by summoning the marker", () => {
    ...
    expect(ray).toContain(
      `execute if block ~ ~ ~ #minecraft:air if score #zzzplugin_grapple_web_steps raycast.work matches 1.. run return run function test:${LOOP}/pass_0`,
    );
    expect(ray).toContain(
    ...
    expect(ray).toContain(
      'summon minecraft:marker ~ ~ ~ {Tags:["grapple.anchor","grapple._new"]}',
    );
    expect(ray).toContain(
      "store result score @s grapple.anchor_x run data get entity @e[distance=..1,tag=grapple._new,limit=1,type=minecraft:marker] Pos[0] 10",
    );
  });
  ...
  it("default reach is 100 steps and anchors on any block (no block gate)", () => {
    ...
    expect(all).toContain(
      "scoreboard players set #zzzplugin_grapple_web_steps raycast.work 100",
    );
  ...
  it("anchorOn restricts the anchor block and maxReach sets the step count", () => {
    ...
    const ray = rayFiles(dp);
    expect(all).toContain(
      "scoreboard players set #zzzplugin_grapple_web_steps raycast.work 60",
    ); // 30 blocks * 2 steps
    // the hit is gated on the block; the summon+read is the raycast plugin's gated on-hit branch.
    expect(ray).toContain(
      "execute if block ~ ~ ~ #minecraft:logs run function test:",
    );
    expect(all).toContain(
      'summon minecraft:marker ~ ~ ~ {Tags:["grapple.anchor","grapple._new"]}',
    );
  ...
  it("start attaches only when an anchor was placed, fixing the rope and leashing it", () => {
    const { dp } = build();
    const start = dp.files.get("zzzplugin/grapple/start")!;
    const all = [...dp.files.values()].join("\n");
    ...
    expect(all).toContain("function test:zzzplugin/grapple/web");
    ...
    expect(attach).toContain(
      "scoreboard players operation @s grapple.rope_len_sq = #to_anchor_x grapple.work",
    );
    expect(attach).toContain(
      "scoreboard players operation @s grapple.rope_len_sq *= #to_anchor_x grapple.work",
    );
    ...
    expect(attach).toContain(
      "scoreboard players operation @s grapple.id = #next_id grapple.const",
    );
    expect(attach).toContain(
      "execute as @e[tag=grapple._new,type=minecraft:marker] run scoreboard players operation @s grapple.id = #next_id grapple.const",
    );
    ...
    const gravityAdd =
      "attribute @s minecraft:gravity modifier add grapple:zero_gravity -1 add_multiplied_total";
    ...
    expect(attach).toContain(
      "tag @e[tag=grapple._new,type=minecraft:marker] remove grapple._new",
    );
  });

  it("drive draws a particle rope: tags this player's anchor, faces it, marches grapple/rope", () => {
    const { dp } = build();
    const drive = dp.files.get("zzzplugin/grapple/drive")!;
    ...
    expect(drive).toContain(
      "execute as @e[tag=grapple.anchor,type=minecraft:marker] if score @s grapple.id = #rope_id grapple.work run tag @s add grapple._aim",
    );
    // aim ^ at it from the eyes and hand off to the marcher, then untag
    expect(drive).toContain(
      "facing entity @e[tag=grapple._aim,limit=1,type=minecraft:marker] feet run function test:zzzplugin/grapple/rope",
    );
    expect(drive).toContain(
      "tag @e[tag=grapple._aim,limit=1,type=minecraft:marker] remove grapple._aim",
    );

    const rope = dp.files.get("zzzplugin/grapple/rope")!;
    ...
    expect(rope).toContain(
      "positioned ^ ^ ^1 run return run function test:zzzplugin/grapple/rope",
    );
  });

  it("constrain assigns the full radial cancel, a Baumgarte trim, and a tangential sustain into the launch input", () => {
    const { dp } = build();
    const c = dp.files.get("zzzplugin/grapple/constrain")!;
    ...
    expect(c).toContain(
      "scoreboard players operation #coef grapple.work -= #dot grapple.work",
    );
    // ... + (dist_sq - rope_len_sq)/BAUMGARTE_DIV (Baumgarte position trim); no `max(0)` floor
    expect(c).toContain(
      "scoreboard players operation #_t0 grapple.work = #dist_sq grapple.work",
    );
    expect(c).toContain(
      "scoreboard players operation #_t0 grapple.work -= @s grapple.rope_len_sq",
    );
    expect(c).toContain(
      "scoreboard players operation #_t0 grapple.work /= #baum_div grapple.const",
    );
    // ... capped at baumMax (the anti-fling bounce killer) before folding into coef
    expect(c).toContain(
      "scoreboard players operation #_t0 grapple.work < #baum_max grapple.const",
    );
    expect(c).toContain(
      "scoreboard players operation #coef grapple.work += #_t0 grapple.work",
    );
    expect(c).not.toContain("#zero");
    // frac = coef * FRAC_SCALE / dist_sq
    expect(c).toContain(
      "scoreboard players operation #frac grapple.work = #coef grapple.work",
    );
    expect(c).toContain(
      "scoreboard players operation #frac grapple.work *= #frac_scale grapple.const",
    );
    expect(c).toContain(
      "scoreboard players operation #frac grapple.work /= #dist_sq grapple.work",
    );
    // impulse = frac * r, written straight into player_motion's launch input
    expect(c).toContain(
      "scoreboard players operation $x player_motion.api.launch = #to_anchor_x grapple.work",
    );
    expect(c).toContain(
      "scoreboard players operation $x player_motion.api.launch *= #frac grapple.work",
    );
    ...
    expect(c).toContain(
      "scoreboard players operation #frac_rad grapple.work = #dot grapple.work",
    );
    expect(c).toContain(
      "scoreboard players operation #frac_rad grapple.work /= #dist_sq grapple.work",
    );
    expect(c).toContain(
      "scoreboard players operation #rad_x grapple.work = #to_anchor_x grapple.work",
    );
    expect(c).toContain(
      "scoreboard players operation #rad_x grapple.work *= #frac_rad grapple.work",
    );
    const tang = "#_t0 player_motion.api.launch"; // the temp on the launch input's objective
    expect(c).toContain(
      `scoreboard players operation ${tang} = #vel_x grapple.work`,
    );
    expect(c).toContain(
      `scoreboard players operation ${tang} -= #rad_x grapple.work`,
    );
    expect(c).toContain(
      `scoreboard players operation ${tang} /= #sustain_div grapple.const`,
    );
    expect(c).toContain(
      `scoreboard players operation $x player_motion.api.launch += ${tang}`,
    );
    // Radial overdamp is disabled (RADIAL_DAMP_DIV=0), so it emits nothing.
    if (RADIAL_DAMP_DIV > 0) {
      expect(c).toContain(
        "scoreboard players operation #rad_x grapple.work /= #radial_damp_div grapple.const",
      );
      expect(c).toContain(
        "scoreboard players operation $x player_motion.api.launch -= #rad_x grapple.work",
      );
    } else {
  ...
  it("drive zeroes the launch, gates the constraint on taut, then clamps and sustains (engine gravity falls)", () => {
    ...
    expect(drive).toContain(
      "execute if score #dist_sq grapple.work >= @s grapple.rope_len_sq run function test:zzzplugin/grapple/constrain",
    );
    ...
    expect(drive).toContain(
      "scoreboard players operation $x player_motion.api.launch < #impulse_max grapple.const",
    );
    expect(drive).toContain(
      "scoreboard players operation $x player_motion.api.launch > #impulse_min grapple.const",
    );
    expect(drive).toContain(
      "scoreboard players set #sustain player_motion.internal.dummy 1",
    );
    expect(drive).toContain("function test:zzzplugin/player_motion/launch_global_xyz");
  });

  it("drive drives every grappling player; stop releases the tag and the player's anchor", () => {
    const { dp } = build();
    expect(dp.files.get("zzzplugin/grapple/tick")).toContain(
      "execute as @a[tag=grappling] at @s run function test:zzzplugin/grapple/drive",
    );
    const stop = dp.files.get("zzzplugin/grapple/stop")!;
    ...
    if (ZERO_GRAVITY) {
      expect(stop).toContain(
        "attribute @s minecraft:gravity modifier remove grapple:zero_gravity",
      );
    } else {
    ...
    expect(stop).toContain(
      "scoreboard players operation #stop_id grapple.work = @s grapple.id",
    );
    expect(stop).toContain(
      "execute as @e[tag=grapple.anchor,type=minecraft:marker] if score @s grapple.id = #stop_id grapple.work run kill @s",
    );
  });
  ...
  it("stop flings the player along their look direction, scaled by swing speed², at @s", () => {
    ...
    expect(all).toContain(
      "scoreboard players operation #frac grapple.work = @s grapple.vel_x",
    );
    expect(all).toContain(
      "scoreboard players operation #frac grapple.work *= @s grapple.vel_x",
    );
    ...
    expect(all).toContain(
      "scoreboard players operation $z player_motion.api.launch = #frac grapple.work",
    );
    expect(all).toContain(
      "scoreboard players operation $z player_motion.api.launch *= #release_kick grapple.const",
    );
    expect(all).toContain(
    ...
    expect(all).toContain("function test:zzzplugin/player_motion/launch_local_xyz");
  });

  it("init seeds the id counter only when unset", () => {
    const { dp } = build();
    const init = dp.files.get("zzzplugin/grapple/init")!;
    ...
    expect(init).toContain(
      "scoreboard players set #release_kick grapple.const 90",
    );
    expect(init).toContain(
      "scoreboard players set #release_kick_max grapple.const 16000",
    );
  });
