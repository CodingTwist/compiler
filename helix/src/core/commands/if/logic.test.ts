...
function build(body: (ctx: FunctionContext, a: ReturnType<typeof scores>) => void, version = v1_21_4) {
...
describe("condition algebra", () => {
  it("folds and() into one execute chain", () => {
    const { f } = build((ctx, { a, b }) => ctx.if(and(a.equal(1), b.atLeast(2)), (c) => c.say("hi")));
    expect(f).toEqual(["execute if score #a v matches 1 if score #b v matches 2.. run say hi"]);
  });

  it("runs an or() body at most once", () => {
    const { f, called } = build((ctx, { a, b }) => ctx.if(or(a.equal(1), b.equal(2)), (c) => c.say("hi")));
  ...
  it("pushes not() through and() with De Morgan", () => {
    const { called } = build((ctx, { a, b }) => ctx.if(not(and(a.equal(1), b.equal(2))), (c) => c.say("hi")));
    expect(called).toEqual([
  ...
  it("compares two scores, with strict literal bounds", () => {
    ...
    expect(f).toEqual(["execute if score #a v < #b v run say lt", "execute if score #a v matches 4.. run say gt"]);
  });

  it("takes a detector, keeping its shifts ahead of the guards they move", () => {
    const pos = Pos.rel(0, 1, 0);
    const { f, called } = build((ctx, { a }) => {
      ctx.if(and(Detect.at(pos, Detect.block(pos, Block.STONE)), a.equal(1)), (c) => c.say("x"));
      ctx.if(not(Detect.at(pos, Detect.entity(Selector.allPlayers()))), (c) => c.say("y"));
    });
    expect(f[0]).toBe("execute if score #a v matches 1 positioned ~ ~1 ~ if block ~ ~1 ~ minecraft:stone run say x");
    expect(f[1]).toBe("execute positioned ~ ~1 ~ unless entity @a[limit=1] run say y");
    expect(called).toEqual([]);
  });

  it("refuses to negate a forking detector", () => {
    expect(() =>
      build((ctx) => ctx.if(not((c) => void c.as(Selector.allPlayers())), (c) => c.say("x"))),
