...
describe("profile report", () => {
  const dp = new Datapack("prof", v1_21_4, undefined, {
    debug: { sources: true },
  });
  let sayHeavy = 0;
  ...
  it("splits self and total time per function, with calls and static command counts", () => {
    ...
    expect(byFn.tick).toMatchObject({
      calls: 3,
      selfNs: 1_500,
      totalNs: 12_500,
      commands: 2,
    });
    expect(byFn.heavy).toMatchObject({
      calls: 2,
      selfNs: 11_000,
      totalNs: 11_000,
      commands: 2,
  ...
  it("maps the hottest commands back to their file line and TS source", () => {
    ...
    expect(
      report.commands.find((c) => c.fn === "other:thing")!.line,
    ).toBeUndefined();
    expect(report.worstTick.commands[0]).toMatchObject({
      fn: "heavy",
      selfNs: 8_000,
    });
