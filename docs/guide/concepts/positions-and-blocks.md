...
One coordinate _mode_ applies to the whole vector - you never mix `~` and absolute within
one `Pos`:
...
`#` (or `Block.tag(...)`) makes a block _tag_ for `if block` / `fill … replace` predicate
