...
const PURE_SHIFTS = new Set<Clause["k"]>([
  "in",
  "positioned",
  "rotated",
  "facing",
...
export function toChains(cond: ExpressionNode): Chain[] {
  ...
  if (cond instanceof PredicateCheckNode)
    return [[{ k: "predicate", mode: "if", id: cond.predicateId }]];
  if (cond instanceof ClausesNode) return [cond.clauses];
  if (cond instanceof OrNode)
    return cond.conds.flatMap((c) => toChains(c as ExpressionNode));
  if (cond instanceof AndNode)
    return product(cond.conds.map((c) => toChains(c as ExpressionNode)));
  // not(a or b) = not a and not b.
  if (cond instanceof NotNode)
    return product(toChains(cond.cond as ExpressionNode).map(negate));
...
function product(lists: Chain[][]): Chain[] {
  return lists.reduce<Chain[]>(
    (acc, list) => acc.flatMap((a) => list.map((b) => join(a, b))),
    [[]],
  );
}
...
function join(a: Chain, b: Chain): Chain {
  ...
  throw new Error(
    "and() can hold only one condition that changes position or executor",
  );
}
...
function negate(chain: Chain): Chain[] {
  ...
  for (const c of chain) {
    if (PURE_SHIFTS.has(c.k)) shifts.push(c);
      ...
      out.push([
        ...shifts,
        { ...c, mode: c.mode === "if" ? "unless" : "if" } as Clause,
      ]);
    } else
      throw new Error(`not() can't negate a condition that uses \`${c.k}\``);
  }
  return out;
}
