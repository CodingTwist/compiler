...
describe("dp.playerMotion (kit)", () => {
  it("emits the apply_impulse enchantment with a leading reset + 96 impulse effects", () => {
    const { dp } = build();
    const ench = dp.registryFileDefs.get(
      "enchantment/internal/apply_impulse",
    ) as {
    ...
    expect(
      impulses.every((e) => e.effect.type === "minecraft:apply_impulse"),
    ).toBe(true);

    // Bit 31 is the sign bit: magnitude -2^31/10000; bit 0 is +0.0001.
    const bit31x = impulses.find(
      (e) => e.requirements?.value?.target?.name === "#x.31",
    )!;
    expect(bit31x.effect.magnitude).toBe(-214748.3648);
    expect(bit31x.effect.direction).toEqual([1, 0, 0]);
    const bit0z = impulses.find(
      (e) => e.requirements?.value?.target?.name === "#z.0",
    )!;
    ...
    expect(
      impulses.every(
        (e) => e.requirements?.value?.score === "player_motion.internal.store",
      ),
    ).toBe(true);
  });
  ...
  it("decomposes each axis into 32 bit lines in internal/store/x", () => {
    ...
    expect(storeX).toContain(
      "store result score #x.0 player_motion.internal.store",
    );
    expect(storeX).toContain(
      "store result score #x.30 player_motion.internal.store run scoreboard players set #x.31 player_motion.internal.store 0",
    );
    expect(storeX).toContain(
      "execute store success score #x.31 player_motion.internal.store if score #x player_motion.internal.dummy matches ..-1 run scoreboard players add #x player_motion.internal.dummy 2147483647",
    );
    expect(storeX).toContain(
      "execute store success score #x.30 player_motion.internal.store if score #x player_motion.internal.dummy matches 1073741824.. run scoreboard players remove #x player_motion.internal.dummy 1073741824",
    );
    expect(storeX).toContain(
      "execute if score #x player_motion.internal.dummy matches 1.. run scoreboard players set #x.0 player_motion.internal.store 1",
    );
  ...
  it("launchGlobal maps x/y/z world axes and calls the global fn", () => {
    ...
    expect(leap).toContain(
      "scoreboard players set $x player_motion.api.launch -15000",
    );
    expect(leap).toContain(
