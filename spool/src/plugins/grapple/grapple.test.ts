...
describe("dp.grapple (kit)", () => {
  ...
  it("the web ray (raycast plugin) marches along ^ while air remains and anchors by summoning the marker", () => {
    ...
    expect(ray).toContain("store result score @s grapple.anchor_x run data get entity @e[distance=..1,tag=grapple._new,limit=1,type=minecraft:marker] Pos[0] 10");
  });
  ...
  it("start attaches only when an anchor was placed, fixing the rope and leashing it", () => {
    ...
    expect(start).toContain("execute if entity @e[tag=grapple._new,limit=1,type=minecraft:marker] run function");
    ...
    expect(attach).toContain("execute as @e[tag=grapple._new,type=minecraft:marker] run scoreboard players operation @s grapple.id = #next_id grapple.const");
    ...
    expect(attach).toContain("tag @e[tag=grapple._new,type=minecraft:marker] remove grapple._new");
  });
  ...
  it("drive draws a particle rope: tags this player's anchor, faces it, marches grapple/rope", () => {
    ...
    expect(drive).toContain("execute as @e[tag=grapple.anchor,type=minecraft:marker] if score @s grapple.id = #rope_id grapple.work run tag @s add grapple._aim");
    // aim ^ at it from the eyes and hand off to the marcher, then untag
    expect(drive).toContain("facing entity @e[tag=grapple._aim,limit=1,type=minecraft:marker] feet run function test:grapple/rope");
    expect(drive).toContain("tag @e[tag=grapple._aim,limit=1,type=minecraft:marker] remove grapple._aim");
    ...
    expect(rope).toContain("unless entity @e[distance=..0.6,tag=grapple._aim,limit=1,type=minecraft:marker]");
  ...
  it("drive drives every grappling player; stop releases the tag and the player's anchor", () => {
    ...
    expect(stop).toContain("execute as @e[tag=grapple.anchor,type=minecraft:marker] if score @s grapple.id = #stop_id grapple.work run kill @s");
  });
