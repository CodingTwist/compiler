...
export function registerGesture<S extends string>(m: MobParts<S>, dp: Datapack, scope: ModuleScope, g: ResolvedGesture<S>): void {
  ...
  if (g.fireAfter) {
    m.add(`${g.name}_hit`, scope.fn(privateName(`${m.name}/${g.name}_hit`), (ctx) => g.onFire?.(ctx, dp, m.handle)));
  }
  if (g.onRecover) {
    const body = g.onRecover;
    m.add(`${g.name}_recover`, scope.fn(privateName(`${m.name}/${g.name}_recover`), (ctx) => body(ctx, dp, m.handle)));
  }
  m.add(
    g.name,
  ...
  if (g.when && g.cooldown >= 5) {
    for (const fn of [g.name, `${g.name}_hit`, `${g.name}_recover`]) {
      if (m.fns.has(fn)) dp.allowNbtRead(m.fns.get(fn)!, `at most once per ${g.cooldown}-tick cooldown`);
    }
  }
  // The clock-driven half runs only for a mob whose clock is running.
  if (g.cooldown !== 0) {
    const clock = m.add(`${g.name}_clock`, dp.createFunction(privateName(`${m.name}/${g.name}_clock`)));
...
function clockGesture<S extends string>(m: MobParts<S>, ctx: FunctionContext, g: ResolvedGesture<S>): void {
  ...
  if (g.sequenced) {
    ...
    const cases = later.map((w, k) => ({
      ...
      fn: m.internal(`${g.name}_step_${k + 1}`, (c) => {
        ...
        if (k === later.length - 1) c.tag().remove(Selector.self(), m.gestureTag(g));
      }),
    }));
    ctx.call(m.internal(`${g.name}_pose`, (c) => c.dispatchScore(m.cooldown(g), cases)));
  }
}

/** Every gesture trigger, behind one finishing check: a finishing mob fires nothing new. */
export function triggers<S extends string>(m: MobParts<S>, ctx: FunctionContext): void {
  ...
  const fire = (c: FunctionContext, g: ResolvedGesture<S>, guard: boolean) => {
    ...
    if (g.cooldown !== 0) chain.unlessScoreMatches(m.cooldown(g), Range.atLeast(1));
    g.when!(chain);
  ...
  if (all.length === 1) {
    ...
    const fn = m.internal("triggers", (c) => all.forEach((g) => fire(c, g, false)));
    ctx
      .execute()
