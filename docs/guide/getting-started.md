...
| `lab` | consumer | Sandbox of concrete features on twine + spool. Builds a playable pack. |
| `unravel` | consumer | Plain-helix worked example, no twine/spool. |
...
profiles, the `Datapack`), and the [plain-helix example](/examples/helix-trivia) for a
full worked pack.
...
framework for composing many features together - the shape `lab` builds on.
