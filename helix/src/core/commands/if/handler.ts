...
export class IfHandler extends CommandHandler<IfElseNode> {
  ...
  private emitBranches(node: IfElseNode, ctx: CodegenContext): void {
    const call = commitLines(
      `${node.thenBody.name}_chain`,
      ctx.datapack,
      this.branchLines(node, ctx),
    );
  ...
  branchLines(
    ...
    const inline = (cmd: string, info: LineInfo) =>
      !info.exits && !FORKS.test(cmd);
    for (const { condition, body } of [
      { condition: node.condition, body: node.thenBody },
      ...node.elifs,
    ]) {
      ...
      for (const clauses of toChains(condition)) {
        this.emitChain(branch, [{ kind: "clauses", clauses }], {
          cmd,
          info: { ...call.info, exits: true },
        });
      }
    }
    if (node.elseBody) {
      const call = generateRunTargetLine(
        node.elseBody,
        dp,
        dispatcher,
        returnElse ? { inline } : {},
      );
      if (call.cmd && returnElse)
        branch.emit(`return run ${call.cmd}`, { ...call.info, exits: true });
  ...
  private emitTracked(node: IfElseNode, ctx: CodegenContext): void {
    ...
    if (!taken)
      throw new Error(
        `if/elif/else on ${ctx.version.id} needs a local, which ctx.if allocates`,
      );
    const { datapack: dp, dispatcher } = ctx;
    ...
    branches.forEach(({ condition }, i) => {
      const mark = generateSingleNodeLine(
        scoreLitNode("set", taken, i + 1),
        dp,
        dispatcher,
      );
      for (const clauses of toChains(condition))
        this.emitChain(ctx, [is(0), { kind: "clauses", clauses }], mark);
    });
    branches.forEach(({ body }, i) =>
      this.emitBodyChain(ctx, [is(i + 1)], body),
    );
  ...
  private emitNodeChain(
    ...
    this.emitChain(
      ctx,
      chain,
      generateSingleNodeLine(node, ctx.datapack, ctx.dispatcher),
    );
  }
