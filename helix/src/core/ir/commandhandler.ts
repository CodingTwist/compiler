...
export class CodegenContext {
  ...
  emit(line: string, info: LineInfo = UNKNOWN_LINE) {
    ...
    this.infos.push(
      macro ? { ...info, clauses: [], open: false, local: false } : info,
    );
...
export class TreeCommandHandler extends CommandHandler<TreeCommandNode> {
  ...
  generate(node: TreeCommandNode, ctx: CodegenContext): void {
    ...
    const onSelf =
      local &&
      onlySelf(node.parts.flatMap((p) => (p.kind === "arg" ? [p.value] : [])));
    ctx.emit(
      buildTokens(ctx.version, tokens),
      commandLine(effect, { exits, local: onSelf }),
    );
  }
}
