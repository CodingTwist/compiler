...
describe("defineMob", () => {
  ...
  it("eases a rise in and holds it, pushing the sequence's step clock back", () => {
    ...
    expect(all).toContain("execute if score @s sentinel.whirl matches 17 run return run function test:sentinel/zzz/whirl_step_1");
    expect(all).not.toContain("sentinel.whirl matches 19 ");
    // ...and so does the fall home, at 20 - 2 - 2.
    expect(all).toContain("execute if score @s sentinel.whirl matches 16 run return run function test:sentinel/zzz/whirl_step_2");
...
describe("mob checks written once", () => {
  const files = (mob: { name: string }) => {
    ...
    class Root {}
    const out = buildDatapack(DatapackFactory.create(Root as never, { name: "test", env: "dev" }));
    return (n: string) => out.get([...out.keys()].find((k) => k.endsWith(`/${n}.mcfunction`))!)!;
  };
  ...
  it("puts every gesture trigger behind one finishing check", () => {
    ...
    const fn = files(
      defineMob(Husk({}), Display(Block.STONE))
        .gesture("a", { members: [0], pivot: [0, 0, 0], rotate: quat("x", 8), when: near })
        .gesture("b", { members: [0], pivot: [0, 0, 0], rotate: quat("x", 9), when: near })
        .toModule("sentinel"),
    );
    expect(fn("tick_one").match(/sentinel\.finishing/g)).toHaveLength(1);
    expect(fn("tick_one")).toContain("execute unless entity @s[tag=sentinel.finishing] run function test:sentinel/zzz/triggers");
  ...
  it("runs a mob's states from one dispatch, timed ones on a clock that keeps it awake", () => {
    const fn = files(
      defineMob(Husk({}), Display(Block.STONE))
        .states({
          up: { polls: 5, onEnter: (c) => c.say("lift"), tick: (c) => c.say("rising"), then: "down" },
          down: { tick: (c, _dp, mob) => c.execute().ifEntity(Selector.self().tag("hit")).run((b) => mob.leave(b)) },
        })
        .gesture("jump", {
          members: [0],
          pivot: [0, 0, 0],
    ...
    expect(fn("enter/up")).toBe(
      ["scoreboard players set @s sentinel.state 1", "scoreboard players set @s sentinel.state_t 5", "say lift"].join("\n"),
    );
    // An untimed state zeroes the clock, or a stale count keeps the mob awake in it.
    expect(fn("enter/down")).toContain("scoreboard players set @s sentinel.state_t 0");
    expect(fn("tick_one")).toContain("execute if score @s sentinel.state matches 1.. run function test:sentinel/zzz/state");
