...
const groupHelix = (sidebar) => {
  ...
  const buckets = new Map(
    [...Object.keys(HELIX_GROUPS), "Other"].map((g) => [g, []]),
  );
  for (const it of items)
    buckets.get(symbolGroup.get(it.text) ?? "Other").push(it);
...
if (!helix) {
  console.error(
    "group-api: no generated API found - run `npm run gen:api` first.",
  );
  process.exit(0);
}
