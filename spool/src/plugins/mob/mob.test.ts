...
describe("mob", () => {
  it("registers on a plain Datapack and exposes wake, tick and its refs", () => {
    ...
    mob.register(dp);
    dp.createFunction("tick", "tick").build((ctx) => mob.tick(ctx));
    dp.createFunction("wake").build((ctx) => ctx.call(mob.wake));
    ...
    expect(text("tick")).toContain("run function test:golem/zzz/tick_one");
    expect(text("golem/zzz/tick_one")).toContain("function test:golem/zzz/face_one");
