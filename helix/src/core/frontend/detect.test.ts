...
function emit(detector: Detector): string[] {
  ...
  return buildDatapack(dp)
    .get("data/t/function/f.mcfunction")!
    .trim()
    .split("\n");
}
...
describe("Detect", () => {
  ...
  it("all() merges into a single chain, in call order", () => {
    expect(
      emit(
        Detect.all(
          Detect.entity(Selector.allPlayers()),
          Detect.block(AT, BUTTON),
        ),
      ),
    ).toEqual([
  ...
  it("in() states the dimension once for the detector beneath it", () => {
    expect(
      emit(Detect.in(Id("minecraft:the_end"), Detect.block(AT, BUTTON))),
    ).toEqual([
  ...
  it("nests without a wrapper chain: composing costs no extra `execute`", () => {
    const deep = Detect.in(
      Id("minecraft:the_end"),
      Detect.near(AT, 8, Detect.block(AT, BUTTON)),
    );
  ...
  it("accepts a hand-written closure wherever a built-in goes", () => {
    const custom: Detector = (c) =>
      void c.ifBlock(AT, BUTTON).ifEntity(Selector.allPlayers());
