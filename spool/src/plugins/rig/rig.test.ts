...
describe("rig", () => {
  it("rides a vehicle and cleans up without a mob", () => {
    ...
    const r = rig(dp.group("cart"), { name: "cart", model: Display(Block.STONE).hitbox(1, 1) });
    const cart = () => Selector.allEntities().tag("cart").limit(1);
    dp.public("spawn").build((ctx) => r.summonOn(ctx, cart));
    dp.public("poll").build((ctx) => {
      r.face(ctx);
      r.face(ctx);
      r.relayHits(ctx, { damage: 2 });
    });
    dp.public("sweep").build((ctx) => {
      r.markOrphans(ctx);
      r.sweep(ctx);
    });
    ...
    expect(text("poll").match(/function test:zzzprivate\/cart\/face_one/g)).toHaveLength(2);
    expect(text("zzzprivate/cart/relay_hit")).toContain("damage @s 2");
    expect(text("sweep")).toContain("tag @e[tag=cart_rig_0,type=minecraft:block_display] add cart.orphan");
    expect(text("zzzprivate/cart/kill_rig")).toContain("on passengers run kill @s");
  });
});
