...
[`Score`](/guide/concepts/scores) lives _on_. You get one from the datapack, and it's
...
Redeclaring a name with a _different_ kind throws - that mismatch is a bug, not something to
paper over.
...
`minecraft.used:<item>` counts how many times the player _used_ (right-clicked) that item -
...
Everything you _do_ with a cell - arithmetic, comparisons, control flow - lives on
