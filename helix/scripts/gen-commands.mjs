...
// Server-console commands: legal in a function, but nothing a datapack can
...
const HAND_REFINED_HANDLERS = ["data", "setblock"];
...
function renderEntry(cmd, methodName, Cmd, emptyGroup, hasOtherGroups, imports) {
  ...
  const lines = [
    ...
    `  const node = new TreeCommandNode(${nodeArgs(cmd)});`,
    `  this.emit(node);`,
...
/** \`${cmd}\` */
export class ${Cmd}Builder extends CommandBuilder<TreeCommandNode> {
${methods.join("\n\n")}
}
...
function main() {
  ...
  generated.sort();
  const barrel = `// Command barrel. Importing each file installs its \`ctx.<cmd>()\`.
...
${HAND_REFINED_HANDLERS.map((c) => `import { ${pascal(c)}Handler } from "./${c}";`).join("\n")}
...
// them.
${[...new Set([...EXTRA_HANDLERS.map((e) => e.module), ...AUGMENT_ONLY])].map((m) => `export * from "./${m}";`).join("\n")}
...
${HAND_REFINED_HANDLERS.map((c) => `    new ${pascal(c)}Handler(),`).join("\n")}
  ];
}
`;
