...
describe("single-target arguments", () => {
  it("throws on a selector that can pick many, and allows one that picks one", () => {
    expect(() => build((ctx) => void ctx.damage().by(Selector.allPlayers(), 2, DamageType.MOB_ATTACK, Selector.self()))).toThrow(
      "`damage` takes one entity",
    );
    expect(build((ctx) => void ctx.damage().by(Selector.allEntities().limit(1), 2, DamageType.MOB_ATTACK, Selector.nearest()))).toBe(
      "damage @e[limit=1] 2 minecraft:mob_attack by @p",
    );
  });
});
...
describe("ctx.setModifier", () => {
  it("removes the modifier before adding it", () => {
    expect(build((ctx) => ctx.setModifier(Selector.self(), Attribute.MOVEMENT_SPEED, Id("t:speed"), 0.2, "add_multiplied_base"))).toBe(
