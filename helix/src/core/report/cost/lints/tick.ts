...
export function nbtWrite({ line, i, period, add }: LineCheck): void {
  ...
  if (typed && linePeriod(line, period!) < NBT_READ_MIN_PERIOD) {
    add(
      ...
      line,
      i,
      `a command does this without serialising the entity: ${typed[1]}`,
    );
  }
}
...
export function collectScans(
  { line, i, sels }: LineCheck,
  state: FnState,
): void {
  for (const s of sels) {
    if (!/@[en]/.test(s.kind) || s.args.length === 0) continue;
    const args = s.args
      .filter(([k]) => k !== "limit" && k !== "sort")
      .map(([k, v]) => `${k}=${v}`)
      .sort();
    ...
    const where = s.args.some(([k]) => POSITIONAL.has(k))
      ? line
          .slice(0, s.start)
          .replace(/\s*(as|at|if entity|unless entity)\s*$/, "")
      : "";
    const key = `${where}|${s.kind}[${args.join(",")}]`;
    const entry = state.scans.get(key) ?? {
      sel: s.text,
      line,
...
export function repeatedSelector(state: FnState, add: AddLint): void {
  for (const { sel, line, lines } of state.scans.values()) {
    ...
    const into = line.startsWith(`execute as ${sel} run function `)
      ? line.split(" ").pop()
      : undefined;
    add(
      "repeated-selector",
...
export function pollTrigger(
  { fn, line, i, sels, add }: LineCheck,
  { criteria, perPlayer }: PackFacts,
): void {
    ...
    ...sels
      ...
      .flatMap((s) =>
        s.args
          .filter(([k]) => k === "scores")
          .flatMap(([, v]) =>
            splitTop(v.slice(1, -1)).map((kv) => kv.split("=")[0].trim()),
          ),
      ),
    ...(asPlayer
      ? [...line.matchAll(/if score @s (\S+) matches/g)].map((m) => m[1])
      : []),
  ];
  ...
  if (trigger) {
    add(
      ...
      line,
      i,
      `polling a stat for a player action: the ${trigger[1]} advancement trigger fires only when it happens`,
    );
  } else if (
    sels.some(
      (s) =>
        ...
        (["x", "y", "z"].some((k) => hasArg(s, k))
          ...
          : hasArg(s, "distance") &&
            /positioned -?[\d.]+ -?[\d.]+ -?[\d.]+ /.test(
              line.slice(0, s.start),
            )),
    )
  ) {
    ...
    new RegExp(
      `if items entity (@a\\S*${asPlayer ? "|@s" : ""}) (container|armor|inventory|hotbar|player\\.crafting)\\.`,
    ).test(line)
  ) {
    add(
      "poll-trigger",
      line,
      i,
      "Trigger.inventoryChanged() fires when a player's inventory changes (a `weapon.*` selection change has no trigger)",
    );
  }
}
