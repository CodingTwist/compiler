...
export function formatCostReport(
  ...
  for (const r of report.tickRoots) {
    ...
    if (r.breakdown.length > 0) {
      if (r.selfCommands > 0)
        out.push(`      · self: ${bold(r.selfCommands)} cmds`);
  ...
  if (report.unboundedScanners.length === 0) {
    ...
    for (const fn of report.unboundedScanners) {
      ...
      for (const loc of new Set(fn.scanSources))
        if (loc) out.push(dim(`      ↳ ${loc}`));
    }
  }
  ...
  const allowed = report.nbtReads.filter(
    (r) => r.allowed && r.period < NBT_READ_MIN_PERIOD,
  );
  if (allowed.length > 0) {
    const why = [...new Set(allowed.map((r) => `${r.fn} (${r.allowed})`))];
    out.push(
      dim(`  ${allowed.length} allowed fast nbt read(s): ${why.join(", ")}`),
    );
  }
  for (const l of report.lints) {
    const line = l.line.length > 120 ? `${l.line.slice(0, 117)}...` : l.line;
    const every = l.period
      ? ` (${l.guarded ? "up to " : ""}every ${l.period} tick(s))`
      : "";
  ...
  for (const l of report.allowedLints) {
    byRule.set(
      l.rule,
      (byRule.get(l.rule) ?? new Set()).add(`${l.fn} (${l.allowed})`),
    );
  }
  for (const [rule, fns] of byRule)
    out.push(dim(`  allowed ${rule}: ${[...fns].join(", ")}`));
  for (const { rule, fn } of report.staleAllows) {
    out.push(
      `  ${warn} dp.allow("${rule}", "${fn}") names no function - renamed or moved? It silences nothing`,
    );
  }
  return out.join("\n");
}
